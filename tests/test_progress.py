...
def test_progress_uses_five_main_phases() -> None:
    ...
    assert progress_for_step(templates, Step.ASK_SALES_SOURCES) == "Tahap 2/5 · Sumber Penjualan"
    assert progress_for_step(templates, Step.ASK_SALES_INPUT) == "Tahap 2/5 · Sumber Penjualan"
    assert progress_for_step(templates, Step.REVIEW_SALES_SUMMARY) == "Tahap 2/5 · Sumber Penjualan"
    assert progress_for_step(templates, Step.EDIT_SALES_MENU) == "Tahap 2/5 · Sumber Penjualan"
    assert progress_for_step(templates, Step.ASK_STOCK_ISSUE) == "Tahap 3/5 · Kendala Stok"
    assert progress_for_step(templates, Step.ASK_NOTE) == "Tahap 4/5 · Catatan"
